...
contract Main {
    ...
    function createLand ( string memory uri_, uint256 price_ ) onlyOwner public returns (uint256) {  // Add Property
        lands[land_count].id = land_count;                                      // Initialize Property
        lands[land_count].uri = uri_;
        lands[land_count].price = price_;
        lands[land_count].max_holder = msg.sender;
        lands[land_count].max_amount = price_;
        lands[land_count].remain = 0;
        lands[land_count].rented = false;
        lands[land_count].renter = dead;
        lands[land_count].holders[msg.sender] = price_;
    ...
    function addLand (uint256 id ) onlyOwner public {  // Add Property
        require(lands[id].listed_rent == false, "This property already listed in rentList");
        lands[id].max_holder = dead;
        lands[id].max_amount = 0;
        lands[id].remain = lands[id].price;
        lands[id].holders[msg.sender] = 0;

        landList.push(id);                                              // When it just added, it will be listed to LandList.
    }
    ...
    function stopRent ( uint256 id ) public {                                   // remove id item from RentList
        ...
        require(lands[id].rent_end_date <= block.timestamp, "Rent is not expired");
    ...
    function calcReward (uint256 id, address user) public view returns (bool, uint256 ) {                 // Calculate rewards
        LAND storage current = lands[id];
        if(current.rented == false || current.rewards[user] == true) {
            return (false, 0);
        }
        if(current.renter == user) {
            return (false, 1);
        }
    ...
    function withdrawReward (uint256 id) public {                                                   // Claim rewards
        ...
        uint256 reward;
        (, reward) = calcReward(id, receiver);                                                  // Calculate reward
